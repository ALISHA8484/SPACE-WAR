...
struct player
{
	...
	char lastmovement;
	int grenade;
} typedef player; // players elements in game
struct profile
{
	int id;
	char username[30] = {'\0'};
...
void signup(users **head)
{
	...
	if (temp->data.id != 0)
	{
		...
		for (; maxfinder != NULL;)
		{
			if (maxfinder->data.id > maxID)
				maxID = maxfinder->data.id;
...
void readhistory(users user, users *head)
{
	...
	for (; fread(&temp, sizeof(history), 1, hhh) > 0;)
	{
		if (temp.fplayerID == user.data.id || temp.splayerID == user.data.id)
		{
			...
			gotoxy(linenum, 22);
			printf("|       ");
			if (strcmp(findusernamewithid(head, temp.firstwinnerID), user.data.username) == 0)
				printf(Green);
			else
				printf(Red);
			...
			gotoxy(linenum, 41);
			printf("|       ");
			if (strcmp(findusernamewithid(head, temp.secondwinnerID), user.data.username) == 0)
				printf(Green);
			else
				printf(Red);
			...
			gotoxy(linenum, 60);
			printf("|       ");
			if (strcmp(findusernamewithid(head, temp.thirdwinnerID), user.data.username) == 0)
				printf(Green);
			else
				printf(Red);
			...
			gotoxy(linenum, 79);
			printf("|       ");
			if (strcmp(findusernamewithid(head, temp.totalwinnerID), user.data.username) == 0)
				printf(Yellow);
			else
				printf(Brown);
	...
	for (int i = 0; i <= 24; i++)
	{
		for (int j = 0; j <= 99; j++)
		{
			...
			if (i == 18)
			{
				...
				if (j == 15 || j == 26 || j == 38 || j == 46 || j == 84 || j == 70 || j == 63)
					map1[i][j] = 8;
			}
			if (i == 19)
			{
				...
				if (j == 14 || j == 26 || j == 38 || j == 46 || j == 83 || j == 70 || j == 63)
					map1[i][j] = 8;
				if (j == 67)
					map1[i][j] = 4;
			}
			if (i == 20)
			{
				...
				if (j == 13 || j == 26 || j == 38 || j == 46 || j == 82 || j == 63)
					map1[i][j] = 8;
			}
			if (i == 21)
			{
				if (j >= 13 && j <= 19)
					map1[i][j] = 9;
				if (j == 27 || (j >= 39 && j <= 41) || j == 54 || j == 64 || j == 65 || j == 71 || j == 72 || (j >= 82 && j <= 88))
					map1[i][j] = 9;
				if (j == 12 || j == 26 || j == 38 || j == 53 || j == 63 || j == 81 || j == 70)
					map1[i][j] = 8;
				if (j == 10)
				{
					map1[i][j] = 11;
			...
			if (i == 22)
			{
				...
				if (j == 11 || j == 26 || j == 38 || j == 46 || j == 80 || j == 70 || j == 63)
					map1[i][j] = 8;
			}
			if (i == 23)
			{
...
int main()
{
	...
	PlaySound(TEXT("./MUSIC/PS2.wav"), NULL, SND_FILENAME | SND_ASYNC);
	printf("\033[48;2;128;0;128m");
	firstwellcome();
	PlaySound(TEXT("./MUSIC/GTA.wav"), NULL, SND_FILENAME | SND_ASYNC);
	...
	for (;;)
	{
		...
		if (y == 1)
		{
			...
			for (;;)
			{
				...
				//
				PlaySound(TEXT("./MUSIC/epic2.wav"), NULL, SND_FILENAME | SND_ASYNC);
				system("cls||clear");
				//
				for (int i = 0; i < 25; i++)
				{
